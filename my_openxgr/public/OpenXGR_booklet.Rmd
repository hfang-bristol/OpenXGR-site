...
if(0){
	...
	knitr::kable(df_source, caption="A summary of primary resources on ontologies and networks that XGR+ relies on.", row.names=T, align='c')
	
}else{
	
	df_source %>% as.data.frame() %>% rownames_to_column('Ontologies or networks') %>% DT::datatable(options=list(pageLength=5,searchHighlight=T,dom='Bfrtip',buttons=c('csv','copy'), dom='t',scrollX=T), style='default', caption="A summary of primary data sources on ontologies and networks that XGR+ relies on.", rownames=FALSE, escape=F, extensions=c('Buttons'))
